...
const checkForParams = async (req: Express.Request, res: Express.Response) => {
  if (Object.entries(req.query).length > 0) {
    const thumbImage = path.resolve(
      thumbnailsPath,
    ...
    fsp
      ...
      .catch(async (e) => {
        try {
          await saveImageToThumbs(
            path.resolve(imagesPath, (req.query.filename as string) + '.jpg'),
            req.query.width as string,
            req.query.height as string,
            thumbImage
          );
          const resizedImg = await sharpResize(
            path.resolve(imagesPath, (req.query.filename as string) + '.jpg'),
            req.query.width as string,
            req.query.height as string
          );
          ...
          res.end(
            'you need to specify valid dimensions and choose image to perform resize opertion on'
          );
        }
      });
  } else {
